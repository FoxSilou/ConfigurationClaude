...
2. Create `src/Api/Program.cs`:
   ...
   - **Bootstrap base Write au démarrage** : dans le bloc `if (!app.Environment.IsEnvironment("Build"))`, appeler `await WriteDatabaseSeeder.EnsureWriteDatabaseAsync(scope.ServiceProvider)` **avant** tout autre seeder. Omettre cet appel provoque `SqlException: Nom d'objet 'StoredEvents' non valide` au premier dispatch de commande. Voir `rules/event-sourcing.md` § "Bootstrap du schéma Write".
   - **Bootstrap base Read au démarrage** : dans le même bloc, appeler `await ReadDatabaseSeeder.EnsureReadDatabaseAsync(scope.ServiceProvider)` **après** `IdentityDataSeeder.EnsureIdentityDatabaseAsync` (Identity crée la base Read + ses tables AspNet ; le seeder Read ajoute ensuite les tables de read models via `CreateTablesAsync` tolérant l'erreur 2714). Omettre cet appel provoque `SqlException: Nom d'objet 'Utilisateurs' non valide` à la première projection. Voir `rules/read-model.md` § "Bootstrap du schéma Read".
3. Create `appsettings.Development.json` with connection strings:
   ...
   ```
   ⚠️ **Exactement deux clés : `Write` et `Read`.** Pas de `"Identity"`, même si un futur BC `Identite` est prévu — les tables `AspNet*` vivent dans la base `_Read`. Voir `rules/identity-framework.md` § "Connection String — partagée avec Read".
