...
## ⚠️ Critical Architecture Rules — Read Before Any Code

These rules are NON-NEGOTIABLE. The scaffold agent must internalize them before writing a single file.

### CQRS — Separate Read / Write Stacks

The project follows **strict CQRS**. Each bounded context has separate Read and Write stacks across all layers. See `cqrs.md` rule.
...
MediatR lives **exclusively** in Infrastructure. See `mediatr.md` rule.
...
Application port signatures must use **domain Value Objects**, not raw primitives. See `port-repository.md`.
...
All aggregate roots inherit from `AggregateRoot<TId>`. See `aggregate.md`.
